...
// Suspend server after 3 days

$vreme = time() + (-3 * 24 * 60 * 60);
mysql_query("UPDATE serveri SET status=Suspended, startovan=0 WHERE istice < '".date("d-m-Y", $vreme)."'");
...
$serveri = mysql_query('SELECT * FROM serveri WHERE status="Suspended"');
...
$serveri = mysql_query('SELECT * FROM serveri WHERE status="Active" AND startovan=1 AND port BETWEEN 26015 AND 39999');
while($row = mysql_fetch_array($serveri)){
$ip = mysql_fetch_array(mysql_query('SELECT * FROM ip WHERE id='.$row["ip_id"].''));	
$test = cs_scan($ip["ip"], $row["port"]);
/*print_r($test);*/
...
if($test["mapname"]=='-'){
$trenutni_status2 = 'Doesn't work';	
} else {
$trenutni_status2 = 'Work';	
}

if($trenutni_status2=='Doesn't work'){
