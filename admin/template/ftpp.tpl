...
<div class="bloc">
    ...
    <div class="title">
    	Server: {$server_info.name} <span style="margin-left: 100px;">IP Adress: {$server_info.ip}:{$server_info.port}</span>
    </div>
    <div class="content">
        <table>
            <thead>
            ...
            {foreach from=$ftp_folderi item=x}
                <tr>
                    <td>		
                    <img src="template/img/ftp_folder.png" border="0" /> <a href="admin.php?page=webftp&id={$id}&path={$old_path}{$x.8}">
             ...
             {foreach from=$ftp_fajlovi item=x}
                <tr>
                    ...
                    {if $x.9} 
                    <a href="admin.php?page=webftp_file&id={$id}&path={$old_path}&file={$x.8}">
                    {if $x.8|count_characters > 18} 
                    {$x.8|substr:0:18}...
                    {else}
                    {$x.8}
                    ...
                    <td class="actions">
                    {if $x.9} 
                    	<a href="admin.php?page=webftp_file&id={$id}&path={$old_path}&file={$x.8}"><img src="template/img/icons/edit.png" border="0" /></a>
                    {/if}                     
